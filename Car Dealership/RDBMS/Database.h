...
public:
	...
	void cross_product(Relation &r1, Relation &r2)
	{
		...
		for(int j = 0; j<r2.tuple_list.size(); j++)
        {
			for(int i = 0; i<r1.tuple_list.size(); i++)
			{
				values = r1.tuple_list[i].get_values();
				for( int k = 0; k<r2.attribute_list.size(); k++){
	...
	void renameAttributes(string attToRename, string value){
        ...
        for (int i=0; i<attribute_list.size(); i++) {
            for (int j=0; j<attToRename.size(); j++) {
				if(attribute_list[i].get_attribute_name().compare(attToRename) == 0) {
	...
	friend ostream& operator<<(ostream& os, Relation r){
		...
		// cell info
		for(int i = 0; i<r.tuple_list.size(); i++)
		{
			os << r.tuple_list[i];
		}
		return os;
